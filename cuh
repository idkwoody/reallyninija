			...
			Event:connect(function()
				local Input = core.InputBegan:connect(function(Key)
					if Key.UserInputType == Enum.UserInputType.MouseButton1 then
						...
						while RunService.RenderStepped:wait() and (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or #UserInputService:GetTouches() > 0) do
...
function Library:slider(options)
	...
	do
        ...
        UserInputService.TouchEnded:connect(function(key)
            down = false
            sliderContainer:tween{BackgroundColor3 = (hovered and self:lighten(Library.CurrentTheme.Secondary)) or Library.CurrentTheme.Secondary}
		end)
